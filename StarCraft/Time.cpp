...
std::unique_ptr<double>			CTime::m_TimeScale = std::make_unique<double>();
...
CTime::CTime()
{
	...
	*m_TimeScale = 1.0 / ((*m_PeriodFrequency).QuadPart);
}
void CTime::Update()
{
	// 현재 프레임 진동수 저장.
	...
	*m_DeltaTime = (float)((*m_CurrentTime).QuadPart - (*m_LastTime).QuadPart) * (*m_TimeScale);
