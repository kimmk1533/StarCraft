...
#pragma region Animation
		...
		const float fps = 1.0f / DEBUG_FPS;
		RECT rect{ 0, 0, 64, 64 };
		std::pair<E_AnimState, E_Direction> animState;
		std::shared_ptr<C_Texture> texture = C_MarineManager::GetI().GetUnitTexture();
...
#pragma region Init
			...
			m_Animator->AddFrame(animState, texture, rect, fps);
#pragma endregion
...
#pragma region Other
			...
			m_Animator->AddFrame(animState, texture, rect, fps * 4);
...
#pragma endregion
	...
	HRESULT C_Marine::Update(const float _deltaTime)
	{
		...
		/*if (m_pInput->KeyDown(E_KeyCode::A))
		{
			m_Animator->SetAnimState(E_AnimState::Walking);
		}
		if (m_pInput->KeyPress(E_KeyCode::A))
		{
			m_Animator->AddPosition(D3DXVECTOR2(-m_Info->fMoveSpeed * _deltaTime, 0.0f));
		}
		if (m_pInput->KeyUp(E_KeyCode::A))
		{
			m_Animator->SetAnimState(E_AnimState::Idle);
		}

		if (m_pInput->KeyDown(E_KeyCode::D))
		{
			m_Animator->SetAnimState(E_AnimState::Walking);
		}
		if (m_pInput->KeyPress(E_KeyCode::D))
		{
			m_Animator->AddPosition(D3DXVECTOR2(m_Info->fMoveSpeed * _deltaTime, 0.0f));
		}
		if (m_pInput->KeyUp(E_KeyCode::D))
		{
			m_Animator->SetAnimState(E_AnimState::Idle);
		}*/
